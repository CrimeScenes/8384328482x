...
-- Prediction for moving targets based on velocity
local function predictPlayerPosition(player, deltaTime)
    local humanoid = player.Character:FindFirstChild("Humanoid")
    local rootPart = player.Character:FindFirstChild("HumanoidRootPart")
    if humanoid and rootPart then
        local velocity = humanoid.RootPart.Velocity
        return rootPart.Position + velocity * deltaTime
    end
    return rootPart.Position
end

-- Adjusting FOV with buffer for moving targets
local function isWithinBoxFOV(position, bufferDistance)
    ...
    return distance <= (math.sqrt((fovHeight / 2)^2 + (fovWidth / 2)^2) + bufferDistance)
end

-- Getting the body parts of a target
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    local predictedPosition = predictPlayerPosition(TargetPlayer, 0.1)  -- Predict position 0.1 seconds ahead
                    local screenPos, onScreen = Camera:WorldToViewportPoint(predictedPosition)
                    if onScreen and isWithinBoxFOV(predictedPosition, 10) then  -- Added buffer distance
                        syncBoxWithTarget(screenPos)
                        if os.clock() - lastClickTime >= 0.05 then  -- Increased delay for smoother firing
